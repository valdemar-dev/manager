...
export default function Accounts() {
  ...
  const [accountToShare, setAccountToShare] = useState<any>(null);
  ...
  const accountShareModal = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    if (!authKey) {
      return router.replace("/user/auth");
    }
    ...
    fetch("/api/user/accounts/getAccounts").then(async (response) => {
      if (!response.ok) {
      ...
      res.forEach((account: any) => {
        account.username = decrypt(account.username, authKey);
        account.email = decrypt(account.email, authKey);
        account.password = decrypt(account.password, authKey);
        account.service = decrypt(account.service, authKey);
      });
      ...
      if (res.length > 0) {
        setAccountsPageMaxIndex(Math.ceil(res.length / accountsPerPage));
      }
  ...
  const addAccount = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    fetch("/api/user/accounts/createAccount", options).then(async (response) => {
  ...
  const handleShowShareModal = (account: object) => {
    setAccountToShare(account);

    accountShareModal.current!.showModal();
  };
    ...
    return filteredAccounts.map((account, index) => {
      return (
        <div className="flex-grow flex items-center gap-2 bg-secondary-200 px-3 py-2 rounded-lg" key={account.id}>
          <span className="mr-auto text-lg">{account.service}</span> 
          <button className="rounded-md bg-secondary-300 duration-200 active:bg-secondary-500 sm:hover:bg-secondary-400 p-2" onClick={() => {copy(account.password)}}>
          ...
          <button className="rounded-md bg-secondary-300 duration-200 active:bg-secondary-500 sm:hover:bg-secondary-400 p-2" onClick={() => {copy(account.username)}}>
            <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <button className="rounded-md bg-secondary-300 duration-200 active:bg-secondary-500 sm:hover:bg-secondary-400 p-2" onClick={() => {copy(account.email)}}>
          ...
          <button 
            className="rounded-md bg-secondary-300 duration-200 active:bg-secondary-500 sm:hover:bg-secondary-400 p-2" 
            onClick={() => {handleShowShareModal(account)}}
          >
            <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M20 13V18C20 19.1046 19.1046 20 18 20H6C4.89543 20 4 19.1046 4 18L4 13M16 8L12 4M12 4L8 8M12 4L12 16" stroke="var(--accent-100)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
        </div>
      )
  ...
  const handleAccountEdit = (event: FormEvent) => {
    ...
    const encryptedUsername = encrypt(usernameField.current!.value, authKey);
    const encryptedUsernameFinal = `${encryptedUsername.iv}${encryptedUsername.authTag}${encryptedUsername.encryptedText}`;

    const encryptedEmail = encrypt(emailField.current!.value, authKey);
    const encryptedEmailFinal = `${encryptedEmail.iv}${encryptedEmail.authTag}${encryptedEmail.encryptedText}`;

    const encryptedPassword = encrypt(passwordField.current!.value, authKey);
    const encryptedPasswordFinal = `${encryptedPassword.iv}${encryptedPassword.authTag}${encryptedPassword.encryptedText}`;

    const encryptedService = encrypt(serviceField.current!.value, authKey);
    const encryptedServiceFinal = `${encryptedService.iv}${encryptedService.authTag}${encryptedService.encryptedText}`;
    ...
    fetch("/api/user/accounts/editAccount", options);

    accountModal!.current.close();
  };

  const handleAccountShare = (event: FormEvent) => {
    event.preventDefault();
    ...
    const encryptedUsername = encrypt(accountToShare.username, authKey);
    const encryptedUsernameFinal = `${encryptedUsername.iv}${encryptedUsername.authTag}${encryptedUsername.encryptedText}`;

    const encryptedEmail = encrypt(accountToShare.email, authKey);
    const encryptedEmailFinal = `${encryptedEmail.iv}${encryptedEmail.authTag}${encryptedEmail.encryptedText}`;

    const encryptedPassword = encrypt(accountToShare.password, authKey);
    const encryptedPasswordFinal = `${encryptedPassword.iv}${encryptedPassword.authTag}${encryptedPassword.encryptedText}`;

    const encryptedService = encrypt(accountToShare.service, authKey);
    const encryptedServiceFinal = `${encryptedService.iv}${encryptedService.authTag}${encryptedService.encryptedText}`;

    const displayname = localStorage.getItem("username");

    const data = {
      accountUsername: encryptedUsernameFinal,
      accountEmail: encryptedEmailFinal,
      accountPassword: encryptedPasswordFinal,
      accountService: encryptedServiceFinal,
    ...
    fetch("/api/user/accounts/sendAccount", options).then(async (response) => {
      setModalText(await response.text());
      showModal(2000);
    });
  }
      ...
      <dialog ref={accountModal} className="rounded-xl">
        <div className="bg-primary-100 text-primary-text drop-shadow-xl p-4 rounded-xl flex flex-col gap-2 animation-delay-1400">
          ...
          <form method="dialog" onSubmit={(event) => {handleAccountEdit(event)}}>
            <span className="text-sm">Username</span>
            <input className="bg-primary-200 focus:bg-primary-300 duration-200 text-primary-text text-lg px-3 py-1 rounded-md w-full mb-2 placeholder-primary-text" name="username" type="username" defaultValue={accountToEdit?.username} ref={usernameField} placeholder="username"/>
            <span className="text-sm">Email address</span>
            <input className="bg-primary-200 focus:bg-primary-300 duration-200 text-primary-text text-lg px-3 py-1 rounded-md w-full mb-2 placeholder-primary-text" name="email" type="email" defaultValue={accountToEdit?.email} ref={emailField} placeholder="email"/>
            <span className="text-sm">Password</span>
            <div className="flex gap-2">
              <input className="bg-primary-200 focus:bg-primary-300 duration-200 text-primary-text text-lg px-3 py-1 rounded-md w-full mb-2 placeholder-primary-text" name="password" required type="password" defaultValue={accountToEdit?.password} ref={passwordField} placeholder="password"/>
            ...
            <span className="text-sm">Service</span>
            <input className="bg-primary-200 focus:bg-primary-300 duration-200 text-primary-text text-lg px-3 py-1 rounded-md w-full mb-2 placeholder-primary-text" name="service" required type="service" defaultValue={accountToEdit?.service} ref={serviceField} placeholder="service"/>
            ...
            <div className="flex gap-2">
              ...
              <button className="bg-secondary-200 text-text active:bg-secondary-400 sm:hover:bg-secondary-300 duration-200 text-lg px-3 py-1 rounded-md w-full mt-3" onClick={() => {accountModal.current!.close()}}>Cancel</button>
            </div>
          </form>
        </div>
      </dialog>

      <dialog ref={accountShareModal} className="rounded-xl">
        <div className="bg-secondary-100 text-text drop-shadow-xl p-4 rounded-xl flex flex-col gap-2 animation-delay-1400">
          <h3 className="text-xl font-semibold text-secondary">Share account</h3>
          <p className="mb-2 unobstructive">You can not undo this action! ManagerX staff will never ask you for your account details.</p>
          <form method="dialog" onSubmit={(event) => {handleAccountShare(event)}}>
            <input className="bg-secondary-200 focus:bg-secondary-300 duration-200 placeholder:text-text text-text text-lg px-3 py-1 rounded-md w-full mb-2" name="email" type="email" placeholder="Email address"/>
            <div className="flex gap-2">
              <button className="bg-primary-200 text-primary-text active:bg-primary-400 sm:hover:bg-primary-300 duration-200 text-lg px-3 py-1 rounded-md w-full mt-3" type="submit">Share</button>
              <button className="bg-secondary-200 text-text active:bg-secondary-400 sm:hover:bg-secondary-300 duration-200 text-lg px-3 py-1 rounded-md w-full mt-3" type="reset" onClick={() => {accountShareModal.current!.close()}}>Cancel</button>
            </div>
          </form>
        </div>
      </dialog>
